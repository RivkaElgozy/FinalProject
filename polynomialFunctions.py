...
def get_number_of_intersections_list_parallel(graph_points, field_p, window):
    ...
    with multiprocessing.Pool(multiprocessing.cpu_count()) as pool:
        ...
        for result in pool.imap_unordered(process_combination_wrapper,((graph_points[a], graph_points[b], field_p, hash_table) for a, b in(random.sample(range(len(graph_points)), 2) for _ in range(total_combinations)))):
            ...
            index += 1
            if index > percent_total_combinations:
                val += index
                index = 0
...
#
#
# def get_number_of_intersections_list(graph_points, field_p, window):
#     counter_values = []
#     hash_table = create_hash_table(graph_points)
#     selected_indexes = []
#     combinations = []
...
#     stop_flag = False
#
#     # Calculate total combinations and collect unique pairs
#     for i, j in itertools.combinations(range(len(graph_points)), 2):
...
#
#     # Process the selected indexes
#     for (i, j) in selected_indexes:
#         if stop_flag:
#             break
#         result = get_intersections_parallel(graph_points[i], graph_points[j], field_p, hash_table)
#         counter_values.append(result)
#
#         # Update the determinate progress bar
#         index += 1
